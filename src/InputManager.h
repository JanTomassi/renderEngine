...
public:
  ...
  static InputManager &
  getInstance () // Singleton is accessed via getInstance()
  {
  ...
  void
  keyboardCallbackImp (GLFWwindow *window, int key, int scancode, int action,
                       int mods)
  {
  ...
  void
  mousePosCallbackImp (
      GLFWwindow *window, double xpos,
      double ypos) // this is the actual implementation of the callback method
  {
    for (auto &input : inputs)
      {
