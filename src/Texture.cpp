...
Texture::Texture (GLuint t, int w, int h)
    : m_tex (genTexture ()), tex_type (t), width (w), height (h)
{
}
...
Texture::setTexture (int level)
{
  ...
  GLCALL (glTexImage2D (tex_type, level, GL_RGBA32F, width, height, 0, GL_RGBA,
                        GL_FLOAT, NULL));
  UnBind ();
}

void
Texture::resizeTexture (int w, int h, int level)
{
  width = w;
  height = h;
  GLCALL (glTexImage2D (tex_type, level, GL_RGBA32F, width, height, 0, GL_RGBA,
...
void
TexParameter::Push (GLuint first, GLuint second)
{
