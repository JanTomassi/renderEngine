...
Window::Window (int x, int y)
    : dim_width (x), dim_hight (y), cam (new Camera ())
{
  ...
  app = GLCALL (
      glfwCreateWindow (dim_width, dim_hight, "Hello World", NULL, NULL));
  ...
  cs = new shader ();
  cs->add (GL_COMPUTE_SHADER, "./shader/ComputeShader.shader");
  cs->compile ();
  cs->useShader ();
...
Window::start ()
{
  ...
  GLCALL (glTexImage2D (GL_TEXTURE_2D, 0, GL_RGBA32F, 640, 480, 0, GL_RGBA,
                        GL_FLOAT, NULL));
  GLCALL (glBindImageTexture (0, tex_output, 0, GL_FALSE, 0, GL_WRITE_ONLY,
                              GL_RGBA32F));
  ...
  while (!glfwWindowShouldClose (app))
    {
      ...
      GLCALL (glTexImage2D (GL_TEXTURE_2D, 0, GL_RGBA32F, width, height, 0,
      ...
      glm::mat4 proj = glm::infinitePerspective (
          glm::radians (120.0f), (GLfloat)width / (GLfloat)height, 0.1f);
