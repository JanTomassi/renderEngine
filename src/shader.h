...
public:
  shader () { m_shader_id = glCreateProgram (); };

  ~shader () { glDeleteProgram (m_shader_id); }

  void
  add (GLuint type, const std::string &src)
  {
    std::ifstream in (src);
  ...
  unsigned int
  getUniLoc (const std::string &name)
  {
    return glGetUniformLocation (m_shader_id, name.c_str ());
  }

  void
  SetUniform4f (const std::string &name, float a, float b, float c, float d)
  {
    glUniform4f (getUniLoc (name), a, b, c, d);
  }

  void
  SetUniformVet2f (const std::string &name, const glm::vec2 &value)
  {
    glUniform2fv (getUniLoc (name), 1, glm::value_ptr (value));
  }

  void
  SetUniformVet3f (const std::string &name, const glm::vec3 &value)
  {
    glUniform3fv (getUniLoc (name), 1, glm::value_ptr (value));
  }

  void
  SetUniformMat4f (const std::string &name, const glm::mat4 &value)
  {
